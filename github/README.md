...
Configure the GitHub App with `Issues: Read and write`, a setup URL of `https://<api-domain>/api/integrations/github/setup`, and a webhook URL of `https://<api-domain>/api/integrations/github/webhook`. The Orcastrate API needs `GITHUB_APP_ID`, `GITHUB_PRIVATE_KEY`, and `GITHUB_WEBHOOK_SECRET`. Each user installs the App from their Orcastrate profile and selects the repositories it may access.
